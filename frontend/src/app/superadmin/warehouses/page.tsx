...
import WarehouseFormModal from '@/app/admin/warehouses/WarehouseFormModal';
import { warehouseService, Warehouse } from '@/services/warehouse.service';
import Link from 'next/link';

export default function SuperAdminWarehousesPage() {
                ...
                filtered.map((w) => (
                  <tr key={w.id} className="border-t border-gray-100 text-gray-700">
                    <td className="px-4 py-3 font-semibold text-gray-900">
                      <Link href={`/superadmin/warehouses/${w.id}`} className="hover:text-[#FF6B35]">
                        {w.name}
                      </Link>
                      {w.isMain && (
